...
model=BAAI/bge-m3
docker run --gpus all -e CUDA_VISIBLE_DEVICES=0  -p 8080:80 -v $volume:/data --pull always ghcr.io/huggingface/text-embeddings-inference:1.5 --model-id $model  --max-batch-tokens 8192 &
docker run --gpus all -e CUDA_VISIBLE_DEVICES=1  -p 8081:80 -v $volume:/data --pull always ghcr.io/huggingface/text-embeddings-inference:1.5 --model-id $model  --max-batch-tokens 8192 &
